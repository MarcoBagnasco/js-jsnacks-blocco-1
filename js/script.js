...
for(var i = 1; i <= request;i++){
    var number = NaN;
    while(isNaN(number)){
        number = parseInt(prompt('Inserisci numero ' + i + ' di ' + request));
    }
    console.log(number);
    sum += number;
    items += '<li>' + number + '</li>';
...
document.getElementById('snack-2-number').innerHTML = numero;

if(numero % 2 === 0){
    console.log('Il numero è pari:', numero);
    document.getElementById('snack-2-print').innerHTML = 'Il numero è pari: ' + numero;
} else{
    document.getElementById('snack-2-print').innerHTML = 'Il numero è dispari. Stampo quello successivo: ' + (numero + 1);
